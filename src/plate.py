...
class Plate(TK.Canvas):
    ...
    def mouse_down(self, event):
        """event bind click;"""
        # shortcuts
        _x, _y = event.x, event.y       # coords
        _t = self.clicked_tile(_x, _y)  # tile
    ...
    def neighboring(self, tile):
        ...
        if len (_i) == 0 and tile%2 == 0:  # tile contiens a number
            return True
        elif len(_i) == 0 and tile%2 == 1: # tile contiens a sign
            return False
        else:
            _l = self._active_tiles[-1]    # last tile actived
